...
- En Android, el core se ejecuta como proceso interno ya supervisado (`STMONITORED=yes`) para evitar el monitor externo/re-exec y SQLite usa cache privado (`SQLITE_TMPDIR`). No se añadió `ACCESS_NETWORK_STATE` solo para aportar un gateway opcional, porque NAT permanece desactivado en esta fase.
